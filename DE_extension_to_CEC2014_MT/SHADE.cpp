...
void algo_SHADE::SingleRun(int r, int _dim, int _pop_size, double _H, double _p, int _func_id,
	bool _archive_flag,
...
void algo_SHADE::RunALG(int _dim, int _pop_size, double _mCR, double _mF, double _H, double _p, int _func_id, bool _archive_flag)
{
	...
	for (int r = 0; r < run; ++r)
	{
		threads.emplace_back(&algo_SHADE::SingleRun, this, r, _dim, _pop_size, _H, _p, _func_id,
			_archive_flag,
			ref(best_fit_record),
			ref(eva_fit_record));
	}
