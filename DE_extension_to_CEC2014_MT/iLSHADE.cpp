...
void algo_iLSHADE::SingleRun(int r, int _dim, int _pop_size, double _H, double _p, int _func_id,
	bool _archive_flag,
	...
	vector<vector<double>>& eva_fit_record)
{
	...
	int t_iter_c = 0;							/* G = 當前迭代次數 */
	int t_dim = _dim;
	int r_init_pop = 18;						/* 初始群體大小比例 */
	double r_archive = 2.6;						/* archive list A 的比例 */
	int t_init_pop_size = t_dim * r_init_pop;	/* 初始群體大小 */
	int t_pop_size = t_init_pop_size;			/* 當前群體大小 */
	int t_min_pop_size = 4;						/* 最小群體大小 = 4 */
	int t_max_archive_size;						/* archive list A 的最大大小 */
	int t_nfes = 0;								/* 初始化評估次數 */
	int t_mnfes = t_dim * 10000;				/* 最大評估次數 */
	vector<double> t_CR;				  /* 存此代每個個體的 CR */
	vector<double> t_F;					  /* 存此代每個個體的 F */
	double t_H = _H;					  /* MCR & MF 大小 */
	double t_p = _p;					  /* top % pop chosen by pbest */
	vector<double> t_MCR;				  /* 存跨越世代的成功mCR */
	vector<double> t_MF;				  /* 存跨越世代的成功mF */
	int t_k = 0;						  /* 要被自適應更新 MCR & MF 的 index */
	...
	while (t_nfes < t_mnfes)
	{
		...
		LPSR(
			t_dim, t_pop_size,
			t_init_pop_size, t_min_pop_size,
			t_mnfes, t_nfes,
			t_pop, t_fit,
			t_A, t_archive_flag, r_archive
		);
		t_iter_c++;			// 更新當前迭代次數
	}
...
void algo_iLSHADE::RunALG(int _dim, int _pop_size, double _mCR, double _mF, double _H, double _p, int _func_id, bool _archive_flag)
{
	...
	vector<vector<double>> eva_fit_record;  // 用於記錄每次運行的 evaluation fitness
	best_fit_record.resize(run);
	eva_fit_record.resize(run);				// 用於記錄每次運行的 evaluation fitness

	vector<thread> threads;					// 用於存放執行緒
	for (int r = 0; r < run; ++r)
	{
		threads.emplace_back(&algo_iLSHADE::SingleRun, this, r, _dim, _pop_size, _H, _p, _func_id,
			_archive_flag,
			ref(best_fit_record),
			ref(eva_fit_record));
	}
...
void algo_iLSHADE::Init(
	...
	int& max_archive_size, double r_archive
)
{
	...
	CR.assign(pop_size, 0.8);	// 初始化 CR = 0.8 (iL-SHADE策略)
...
)
{
	...
	if (p_min < 0.2) {
		uniform_real_distribution<double> dist_p(p_min, 0.2);
		p_i = dist_p(gen);
	}
	else {
	...
	for (int i = 0; i < pop_size; ++i)
	{
		...
		/* 生成此代所有個體的 CR[i] & F[i] */
		CR[i] = max(0.0, min(1.0, dist_CR(gen))); // CR[i] truncate, 確保 CR[i] 在 [0, 1] 範圍內
		...
		do {
			F[i] = dist_F(gen);
		} while (F[i] <= 0.0 || F[i] > 1.0);      // F[i] truncate
...
void algo_iLSHADE::LPSR(
	int dim, int& pop_size,
	int init_pop_size, int min_pop_size,
	int mnfes, int nfes,
	vector<vector<double>>& pop, vector<double>& fit,
	vector<vector<double>>& A, bool archive_flag, double r_archive
)
{
	random_device rd;
	mt19937 gen(rd());
