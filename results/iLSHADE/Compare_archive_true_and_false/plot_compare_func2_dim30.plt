...
iLSHADEfile1 = sprintf("iLSHADE_fitness_avg_cvg%d_dim%d_archive_%s.txt", func_id, dim, archive)
iLSHADEfile2 = sprintf("iLSHADE_fitness_avg_cvg%d_dim%d_archive_false.txt", func_id, dim)
set output outname
...
# === TITLE AND AXIS ===
set title sprintf("Comparison of iL-SHADE on f%d (dim=%d)", func_id, dim) \
...
plot \
    iLSHADEfile1 with lines linewidth 2 linecolor rgb '#DC143C' title 'iL-SHADE',\
    iLSHADEfile2 with lines linewidth 2 linecolor rgb '#6495ED' title 'iL-SHADE w/o archive'